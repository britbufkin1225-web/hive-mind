...
try {
    ...
    Add-Test '23 valid manifest with empty lists passes' { $f=New-Fixture -Parent (Join-Path $tempRoot 'manifestvalid'); $path=Join-Path $f.Path 'manifest.json'; New-ManifestObject $f | ConvertTo-Json -Depth 10 | Set-Content $path; $a=Get-Arguments $f; $a.ManifestPath=$path; $r=Invoke-HiveMindGovernancePreflight @a; $blocked=@($r.Checks|Where-Object {$_.CheckIdentifier -like 'manifest*' -and $_.Status -eq 'BLOCKED'}); Assert-True ($blocked.Count -eq 0) ('Contract-valid manifest with empty lists was blocked: '+(($blocked|ForEach-Object CheckIdentifier) -join ', ')); Assert-True ($r.OverallResult -eq 'PASS') 'Contract-valid manifest did not yield overall PASS.' }
    Add-Test '24 remote fallback strips only a single .git suffix' { Assert-True ((ConvertTo-NormalizedRemote 'sample.git') -eq 'sample') 'A trailing .git suffix was not stripped in the fallback branch.'; Assert-True ((ConvertTo-NormalizedRemote 'foo-git') -ne (ConvertTo-NormalizedRemote 'foo-tig')) 'The fallback over-trimmed trailing g/i/t characters and collapsed distinct remotes.' }
    Add-Test '25 competing manifest version is blocked' { $f=New-Fixture -Parent (Join-Path $tempRoot 'manifestversion'); $path=Join-Path $f.Path 'manifest.json'; $obj=New-ManifestObject $f; $obj.manifest_version='competing-schema.v99'; $obj | ConvertTo-Json -Depth 10 | Set-Content $path; $a=Get-Arguments $f; $a.ManifestPath=$path; $r=Invoke-HiveMindGovernancePreflight @a; Assert-True (Has-Blocked $r 'manifest.version.manifest_version') 'A competing manifest_version was accepted.' }
}
