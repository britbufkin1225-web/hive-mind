...
- **Planned:** active-state calculation, repository observation API, evidence resolution, ingestion, and any persistent Active Memory runtime.
...
| Repository snapshot service | Implemented | Backend-only request-triggered service over the Git adapter; no API, watcher, persistence, ingestion, frontend, or mutation. |
| Active Memory runtime | Planned | Active-state calculation, write endpoints, observation API, durable memory, ingestion, and evidence resolver are not implemented. |
...
Hive|Mind is currently a local, single-user developer tool. It has no authentication, authorization, multi-user support, cloud sync, or production deployment hardening. Obsidian import is explicit and one-shot; there is no live vault watcher and no write-back. The Knowledge Graph and Intelligence Report are read-only, and suggestions are advisory only. Query-history persistence remains absent, so query-history-dependent categories stay deferred. The Active Memory store is deterministic but in-memory only (a serialize/restore boundary, no committed persistence medium); contradiction detection is backend-derived, read-only, and covers four of the five contract classes (`frontend_only_vs_backend_modification` is deferred, and no automatic resolution exists); the context packet endpoint and inspector are read-only and stateless — they derive packets from records explicitly supplied in the current request, with no server-side memory store, persistence, ingestion, repository observation API, evidence resolver, AI interpretation, action authorization, or mutation controls. The Repository Observer now has backend schema contracts, a deterministic read-only Git adapter foundation, and a backend-only request-triggered snapshot service, but no observation API, watcher, polling loop, persistence, ingestion, frontend visibility, or production-ready runtime. Gesture tracking remains experimental and needs live tuning. The product does not run autonomous agents or mutate repositories.

## Roadmap

The current controlled Track 2 sequence after the completed Phase 37K snapshot service MVP is:

```text
37L - Repository Observation API Foundation
```

Phase 37K adds a backend-only repository observation snapshot service from the [repository observer plan](docs/planning/phase-37h-repository-observer-planning.md). The service uses the Phase 37J adapter for allowlisted Git metadata commands and low-level conversion helpers, owns request-triggered snapshot orchestration, applies conservative scope limits, and returns the existing `repo-observer.v1` `RepositorySnapshot` contract. It does not add a filesystem scanner, watcher, endpoint, persistence, Active Memory ingestion behavior, GitHub integration, frontend surface, or repository mutation. Phase 36K remains paused, not canceled or completed. Gesture tuning can resume after the application's memory foundation reaches a usable state. The complete phase chronology belongs in the [roadmap](docs/roadmap.md).
