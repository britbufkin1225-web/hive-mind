...
| Memory migration reviewed persistence and import | Planning in progress (documentation-only) / no runtime | Reviewed persistence and verified import are not implemented (Phase 40H). The [Phase 40H plan](docs/planning/phase-40h-reviewed-persistence-verified-import-planning.md) defines a single reviewed-import service as the exclusive Active Memory mutation boundary, the durable record types, review-decision provenance (a bare approval boolean never authorizes mutation), the verified-import preconditions (reload candidate, recompute/compare byte digest, confirm assessment identity/version, require complete non-contradictory approval, fail closed), a deterministic import receipt, idempotency/replay and atomicity/recovery rules, typed diagnostics, a test matrix, and a bounded file map. It proposes reusing the established local versioned-JSON persistence architecture and explicitly rejects a server-database rewrite; no new public API is proposed. Planning-only: nothing is implemented and no candidate is persisted or imported into Active Memory today; implementation requires separate approval. Hive\|Mind cannot directly access private ChatGPT system memory; migration relies on user-controlled exports or curated bundles. |
