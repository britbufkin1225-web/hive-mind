...
Hive|Mind is also developing an **Active Memory and Verification** architecture: a contract-first layer for tools and agents to read verified, evidence-linked project context before acting. Its implemented foundation includes deterministic context packets and a read-only inspector, plus a Repository Observer with bounded snapshot and drift APIs, deterministic repository-evidence projection into candidate memory records, a persistent local repository-workspace registry, and a one-command managed local runtime. Phase 40E — the memory-migration **intake boundary**: contracts plus a deterministic, read-only assessment over declared metadata — is merged. Phase 40F — the memory-migration **export parser and candidate projector** — is merged: holding a matching Phase 40E `ready_for_parsing` assessment, it reads the declared bytes, verifies they match the declared digest and size, safely parses the supported export and curated formats, and projects user-visible content into **inactive, unverified, provenance-linked candidates**. Nothing is persisted or imported into Active Memory: a read-only Phase 40G dry-run assessment gates the projected candidate set, and reviewed persistence and verified import remain the exclusive Phase 40H boundary (a **documentation-only** plan that has **already selected** its durable medium — reuse of the established local versioned-JSON persistence: a workflow ledger plus a mandatory Active Memory snapshot store bound by a shared commit generation — with **no runtime implemented, no public API, local and unmerged, implementation locked**, and pending a **final independent seven-commit audit**) — byte-integrity means only that the bytes are the declared bytes, never that their contents are true. Multi-agent contribution governance (Agent Lab) is defined and enforced locally through a dependency-free PowerShell preflight.
...
and the [roadmap Grounded Synthesis + Memory Migration track](docs/roadmap.md#grounded-synthesis--memory-migration-track-planned).
...
| Grounded Synthesis contracts | Implemented / merged | Phase 40B backend-only `grounded-synthesis.v1` contract and schema foundation: synthesis modes (`musings`, `loom` — capabilities within the layer, not layers), request, grounding evidence references, constraints, context packet, proposed artifact, mandatory provenance, validation results, and a bounded readiness vocabulary with no accepted/approved/committed/applied state. Deterministic and caller-clock-owned; no synthesis behavior, producer, endpoint, frontend, persistence, migration, repository write, graph mutation, Active Memory insertion, or AI/LLM integration. |
| Grounding context assembly | Implemented / merged | Phase 40C backend-only deterministic, read-only `GroundingContextAssemblyService` that assembles five existing evidence families (Active Memory evidence records, Repository Observer observations, repository drift findings, contradiction records, Active Memory records) into valid Phase 40B `SynthesisContextPacket` records: explicit eligibility filtering, canonical-identity deduplication with documented winner precedence, criticality-first stable ranking, per-family and packet bounds with represented truncation, surfaced-never-resolved conflicts, deterministic readiness, and bounded secret-free diagnostics. Contracts reused unchanged; raw provider payloads, absolute roots, and remote URLs are never copied. No endpoint, persistence, cache, packet history, frontend, dependency, mutation, or AI/LLM/synthesis generation. |
| Grounded Synthesis validation guardrails | Implemented / merged | Phase 40D backend-only deterministic, read-only `SynthesisContextPacketValidator` deciding whether an assembled packet is trustworthy enough for a future synthesis capability: canonical evidence identity guardrails reusing the assembler's own rule, provenance integrity and resolution checking, fail-closed repository/source safety, packet metadata reconciled against actual contents (evidence and coverage totals, conflict totals, readiness reasons, canonical ordering, and a re-derived content-addressed packet identity), bounds and truncation validation that never clips, blocking and non-blocking diagnostics with severity fixed by the code, and an explicit synthesis-readiness determination projected onto the canonical Phase 40B `SynthesisValidationResult`. Diagnostics carry counts, closed-enum literals, and packet-local identifiers only — never a path, remote, credential, or provider payload. Phase 40B and 40C contracts reused unchanged. No endpoint, persistence, frontend, dependency, mutation, or AI/LLM/synthesis generation. |
| Memory migration intake safety | Implemented / merged | Phase 40E backend-only `memory-migration.v1` contracts plus a deterministic, pure, read-only `MemoryMigrationIntakeAssessor` over **declared metadata only**. Fail-closed allowlists for custody, source identity, declared path shape, entry type, format/container, digest algorithm and bounds; declared counts and totals recomputed and reconciled; content-derived bundle and artifact fingerprints that bind a verdict to the exact declaration; a derived `declared → ready_for_parsing / blocked / quarantined` status a caller cannot assert for itself; and bounded diagnostics that never echo a declared path, media type, digest, or label. Raw bundles pin `is_memory` and `is_verified_evidence` to false, declared digests pin `verified` to false, and candidate memory is pinned inactive, unverified, and non-persistable. Nothing is repaired, clipped, or dropped. No byte, filesystem, archive, Git, or network access; no endpoint, router, frontend, store, persistence, dependency, or AI/LLM integration. |
| Memory migration parsing and candidate projection | Implemented / merged | Phase 40F backend-only export **parser** (the only component permitted byte access) and **pure projector**. Reuses the exact Phase 40E `permits_parsing(bundle_fingerprint=…)` gate — no byte is read before it passes and a stale/mismatched assessment authorizes nothing — verifies actual byte integrity (recomputed accepted digest and size) without mutating the Phase 40E declared-digest flag, parses `chatgpt_export_archive` (defensive in-memory ZIP, never extracted to disk), `chatgpt_conversations_json`, `curated_json_bundle`, `curated_markdown_bundle`, and `plain_text_document` deterministically, and projects only user-visible `user`/`assistant` and document content into inactive, unverified, human-review-required, non-persistable, provenance-linked candidates bound by `CANDIDATE_MEMORY_POLICY`. Deterministic content-derived identity/ordering; bounded content with deterministic chunking; honest overflow; closed, code-derived-severity diagnostics carrying no raw exported content. No persistence, Active Memory insertion, verified/confirmed candidate, activation, endpoint, router, frontend, Source Registry/graph/repository/Git mutation, network, new dependency, or AI/LLM integration. |
| Memory migration candidate dry-run assessment | Implemented / merged | Phase 40G backend-only deterministic, pure, read-only `MemoryMigrationCandidateAssessor` over a *complete* Phase 40F candidate set and its projection diagnostics: exact duplicates grouped and counted (never removed), conflicting source identities flagged as blocking, ambiguous ordering and empty/degenerate candidates noted as advisory, unresolved Phase 40F errors carried as blocking and overflow/truncation as advisory with original severity preserved, bounded aggregate coverage, a content-addressed tamper-evident report identity, and a derived `ready_for_review` / `review_with_warnings` / `blocked` verdict that fails closed. Phase 40F contracts reused unchanged. Nothing is persisted, imported, verified, approved, activated, ranked, deleted, or mutated; no endpoint, router, frontend, persistence, database, Active Memory, dependency, or AI/LLM integration; no clock, randomness, or I/O (AST-enforced). |
| Memory migration reviewed persistence and import | Documentation-only planning (local, unmerged) / no runtime / no public API | Reviewed persistence and verified import are not implemented (Phase 40H). The [Phase 40H plan](docs/planning/phase-40h-reviewed-persistence-verified-import-planning.md) is **documentation-only and has already selected its durable medium**: a two-store model in which the **existing Active Memory store stays authoritative** for memory records and a new reviewed-import service acts as an **orchestration boundary** — the sole path that inserts an approved candidate through the Active Memory store's own seam, while a separate durable ledger owns only migration-workflow records (review decisions, import attempts, receipts, **authorization revocation entries**, idempotency/recovery metadata) and references the resulting `record_id` rather than copying it. The plan also defines review-decision provenance (a bare approval boolean never authorizes mutation); a mandatory reviewer-approved **import specification** that supplies the record `kind`, structured `claim`, and validated `project_id` that the Phase 40F candidate does **not** carry (the coordinator validates and executes it and never infers those fields); a typed **project/scope authorization context** whose expiration is evaluated by a **trusted server-side UTC clock** and whose revocation is an **authoritative durable ledger registry** (never a caller-presented boolean), with a deterministic issuance discriminator and a single canonical project-mismatch diagnostic; a mandatory **authoritative live-store holder** that owns the live Active Memory reference and performs atomic, reader-safe store replacement; the verified-import preconditions (reload candidate, recompute/compare byte digest, confirm assessment identity/version, require complete non-contradictory approval, fail closed); a deterministic import receipt whose **full-content integrity digest covers both timestamps** (identity kept separate from integrity); a **recoverable cross-store transaction protocol that does not claim single-swap atomicity**, including an explicit **N/N+1 uncertain-commit recovery exception** evaluated before ordinary generation equality; an exclusive-writer + persisted-revision/CAS concurrency mechanism; a stable idempotency key with distinct monotonic attempt ids; uncertain-commit recovery; typed diagnostics; a test matrix; and an integration map naming the existing Active Memory modules it touches. It reuses the established local versioned-JSON persistence architecture and explicitly rejects a server-database rewrite; **no new public API is proposed and no instruction remains to choose a durable medium**. Documentation-only: nothing is implemented and no candidate is persisted or imported into Active Memory today; the plan is **local and unmerged** (now **seven commits ahead of `origin/main`** after this hardening commit), **implementation remains locked**, and it is **pending a final independent seven-commit audit** — no claim is made that it has passed. Phase 36K remains **paused and untouched**. Hive\|Mind cannot directly access private ChatGPT system memory; migration relies on user-controlled exports or curated bundles. |
...
`main`, and Phase 40A opened the Grounded Synthesis track. Its contract,
assembly, and validation foundation (Phases 40B–40D) is now complete, and
Phase 40D.5 reconciles the roadmap to a memory-migration pivot:
...
40B - Grounded Synthesis Contract Types + Schema Foundation (merged)
40C - Grounding Context Assembly Service MVP (merged)
40D - Synthesis Evidence, Provenance, and Validation Guardrails (merged)
40D.5 - Roadmap Reconciliation + Memory Migration Pivot (documentation-only)
40E - Memory Migration Contract + Intake Safety Foundation (merged)
40F - Export Parser + Candidate Projection (merged)
40G - Migration Candidate Assessment (Dry-Run) (merged)
40H - Reviewed Persistence + Verified Import (planning; merged)
40I - Reviewed Persistence + Verified Import Implementation (merged)
40J - Migration Rehearsal + End-to-End Validation (disposable data only; local, pending audit)
```

Phase 40J validates the complete reviewed migration path against deterministic
synthetic disposable data. The authoritative dataset was not accessed or mutated,
and no real migration occurred. The rehearsal passed parsing, projection,
...
Phases 38A–39D are merged: Agent Lab multi-agent [contribution contracts](docs/agent-lab/README.md) and a [local PowerShell governance preflight](scripts/governance/README.md); deterministic repository-evidence projection into candidate memory records; a persistent local [repository-workspace registry](scripts/workspaces/README.md); a one-command [managed local runtime](docs/operator-runtime.md); and Repository Observer [end-to-end workflow hardening and failure-state QA](docs/operator-repository-observer.md). Phase 40A defined the **Grounded Synthesis Layer** foundation, Phase 40B added its backend `grounded-synthesis.v1` contract and schema foundation, Phase 40C added the deterministic, read-only grounding context assembly service over those contracts, and Phase 40D added the deterministic, read-only validation guardrails that decide whether an assembled packet is trustworthy enough to hand onward (see [Direction: the Grounded Synthesis Layer](#direction-the-grounded-synthesis-layer-contracts-grounding-assembly-and-validation-guardrails)); no synthesis generation, producer, endpoint, frontend, persistence, or AI/LLM integration exists. Phase 40D.5 is a documentation-only pivot: it sequences a **memory-migration track ahead of Grounded Synthesis production**, so a user's own history can enter Active Memory as reviewable candidates before a synthesis producer is built. Phase 40E adds that track's **intake boundary** (merged): the backend `memory-migration.v1` contract family and a deterministic, read-only assessment that judges a migration bundle's **declared metadata** and derives `ready_for_parsing`, `blocked`, or `quarantined` — a status a caller cannot assert for itself. Phase 40F adds the track's **export parser and candidate projector** (merged): holding a matching `ready_for_parsing` assessment, it reads the declared bytes, proves they match the declared digest and size, safely parses the supported export and curated formats, and projects user-visible conversational and document content into **inactive, unverified, provenance-linked candidates**. **Nothing is persisted or imported today**: parsing is interpretation, not verification, activation, or persistence — byte-integrity means only that the bytes are the declared bytes, never that their contents are true. Phase 40G adds a deterministic, read-only **dry-run assessment** over the complete projected candidate set — grouping duplicates, flagging source conflicts, and deriving a review-readiness verdict without persisting or mutating anything — and reviewed persistence and verified import remain the exclusive Phase 40H boundary, now in **documentation-only** [planning](docs/planning/phase-40h-reviewed-persistence-verified-import-planning.md) that **has already selected its durable medium** (an orchestration in which the existing Active Memory store stays authoritative and one reviewed-import coordinator inserts approved candidates through that store's own seam — reached through a **mandatory authoritative live-store holder** that performs atomic, reader-safe store replacement — using a mandatory **reviewer-approved import specification** to supply the record kind, structured claim, and validated project id the Phase 40F candidate does not carry (validated, never inferred), backed by a separate durable workflow ledger — which also owns an **authoritative durable authorization-revocation registry** and evaluates authorization expiry against a **trusted server-side UTC clock** — and a **mandatory durable Active Memory snapshot store** bound to it by a shared persisted **commit generation**, all serialized by a single coordinator lock with a recoverable — never falsely atomic — cross-store protocol including an explicit **N/N+1 uncertain-commit recovery exception**; reuse of the established local versioned-JSON persistence rather than a server database; with no candidate persisted or imported today). The persistence protocol is **local and unmerged**, has **no runtime and no public API**, its **implementation remains locked**, and it is **pending a final independent seven-commit audit** (another Codex re-audit) that it has **not** passed; **no instruction remains to choose a durable medium**. Hive|Mind cannot directly access private ChatGPT system memory; migration relies on **user-controlled exports or curated bundles** the user provides — see the [Phase 40D.5 decision record](docs/planning/phase-40d-5-roadmap-reconciliation-memory-migration-pivot.md), the [Phase 40E record](docs/planning/phase-40e-memory-migration-contract-intake-safety-foundation.md), the [Phase 40F plan](docs/planning/phase-40f-export-parser-candidate-projection.md), and the [Phase 40G plan](docs/planning/phase-40g-migration-candidate-assessment-dry-run.md). Phase 36K remains paused, not canceled or completed. Active Memory continues to govern project data and verification architecture while Agent Lab governs contribution workflow. The complete chronology and the Grounded Synthesis + Memory Migration track belong in the [roadmap](docs/roadmap.md).
...
- [Phase 40D.5 roadmap reconciliation + memory migration pivot](docs/planning/phase-40d-5-roadmap-reconciliation-memory-migration-pivot.md)
- [Phase 40E memory migration contract + intake safety foundation](docs/planning/phase-40e-memory-migration-contract-intake-safety-foundation.md)
- [Phase 40F export parser + candidate projection](docs/planning/phase-40f-export-parser-candidate-projection.md)
- [Phase 40G migration candidate assessment (dry-run)](docs/planning/phase-40g-migration-candidate-assessment-dry-run.md)
- [Phase 40H reviewed persistence + verified import (planning)](docs/planning/phase-40h-reviewed-persistence-verified-import-planning.md)
