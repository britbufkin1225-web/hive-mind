...
rather than a server database, with no runtime implemented and pending a further
...
durable versioned-JSON workflow ledger and a mandatory durable Active Memory snapshot
store bound to it by a shared persisted commit generation, serialized by a single
coordinator lock under a recoverable — never falsely atomic — cross-store protocol)
with no runtime implemented, proposed and pending a further independent audit.
