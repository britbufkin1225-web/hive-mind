...
Phase 40H boundary (whose documentation-only plan now proposes a concrete durable
medium: reuse of the established local versioned-JSON persistence — a workflow ledger
plus a mandatory Active Memory snapshot store bound by a shared commit generation,
...
does not carry — rather than a server database, with no runtime implemented and
pending a further independent audit). See the
...
Phase 40H, whose documentation-only plan proposes an orchestration (the
existing Active Memory store stays authoritative; one reviewed-import coordinator
inserts approved candidates through that store's own seam via a mandatory
...
separate durable versioned-JSON workflow ledger and a mandatory durable Active Memory
snapshot store bound to it by a shared persisted commit generation, serialized by a
single coordinator lock under a recoverable — never falsely atomic — cross-store
protocol with an explicit N/N+1 uncertain-commit recovery exception) with no runtime
implemented, proposed and pending a further independent audit.
