...
### Phase 40F — Export Parser + Candidate Projection (backend service phase, merged)

Phase 40F is **merged** and is the first memory-migration phase
permitted to read user-controlled artifact **bytes**. It is backend-only and adds
two components: an export **parser** (`apps/backend/app/services/memory_migration_parser.py`,
the only Phase 40F component permitted filesystem/archive/byte I/O) and a **pure,
deterministic projector** (`apps/backend/app/services/memory_migration_projection.py`),
with the candidate/result contracts in
`apps/backend/app/models/memory_migration_projection.py`.

It reuses the exact Phase 40E authorization gate —
`assessment.permits_parsing(bundle_fingerprint=bundle.fingerprint())` — and reads
no byte until it passes; a stale or mismatched assessment authorizes nothing.
Before trusting an artifact it recomputes the declared (accepted) digest and size
over the **actual bytes** and fails closed on any mismatch, without ever mutating
the Phase 40E declared-digest flag. It parses the supported ChatGPT export and
curated formats deterministically — the ChatGPT archive is read defensively in
memory and never extracted to disk — and projects only user-visible
`user`/`assistant` conversational text and user-authored documents into
**inactive, unverified, human-review-required, non-persistable, provenance-linked
candidates** structurally bound by `CANDIDATE_MEMORY_POLICY`.

Parsing is interpretation, not authority. Phase 40F **persists nothing, inserts
nothing into Active Memory, verifies no claim, confirms nothing, and activates
...
Phase 40H boundary (whose **documentation-only** plan has **already selected** its
durable medium: reuse of the established local versioned-JSON persistence — a workflow
ledger plus a mandatory Active Memory snapshot store bound by a shared commit generation,
reached through a mandatory authoritative live-store holder, with a reviewer-approved
import specification supplying the record kind/claim/project the Phase 40F candidate
does not carry — rather than a server database, with **no runtime, no public API, local
and unmerged, implementation locked**, and pending a **final independent seven-commit
audit** it has not passed). See the
...
| --- | --- | --- |
| Phase 40I — Reviewed Persistence + Verified Import Implementation MVP | Implemented locally / pending independent audit | Implements the merged Phase 40H contract as a backend-only, human-gated coordinator. Active Memory remains authoritative; a separately integrity-sealed workflow ledger and Active Memory snapshot share a commit generation; imports persist intent, publish the durable effect, persist and verify one immutable receipt, then replace the live store last. Exact replay returns the stored receipt unchanged. Authorization uses exact project/scope bindings, trusted-clock expiration, and durable revocation. Corrupt, conflicting, incomplete, or ambiguous states fail closed and quarantine. No frontend, public API, synthesis producer, automatic promotion, LLM adjudication, or Phase 36K work. This row supersedes the historical Phase 40H planning-only status and the older provisional use of “40I” for the synthesis producer. |
| Phase 40J — Migration Rehearsal + End-to-End Validation | Implemented locally / pending independent audit | Exercises the real Phase 40F–40I parser, projection, assessment, reviewed import, durable ledger/snapshot, receipt, replay, authorization, CAS, integrity, and N/N+1 recovery seams against deterministic synthetic disposable data only. Adds a fail-closed authoritative-path collision guard and exact safe reset. Authoritative dataset access was zero; no real migration occurred. Passing makes an authoritative migration eligible only for a separate human-authorized phase. Phase 36K remains paused and untouched. See the [Phase 40J evidence](qa/phase-40j-migration-rehearsal-end-to-end-validation.md). |
| Phase 40K — Authoritative Migration Runbook + Execution Readiness | Implemented locally / operational readiness blocked / pending independent audit | Adds the canonical [runbook](operations/phase-40k-authoritative-migration-runbook.md), objective [operator checklist](operations/phase-40k-operator-checklist.md), and fail-closed [readiness template](operations/phase-40k-readiness.template.json). It translates Phase 40F–40J implementation truth into dataset identity, backup/restoration, authorization, preflight, stop, evidence, recovery, and acceptance controls. No production migration ran and no real operational facts were fabricated. Phase 40L remains conditional on a separate human go decision after every blocker is verified. |
| Phase 40K.5 — Production Migration Interface + Operational Readiness Remediation | Merged (PR #203) / operational readiness blocked / pending independent audit | Resolves the Phase 40K tooling blocker: a genuinely read-only [migration readiness preflight](operations/phase-40k-5-migration-readiness-interface.md) (`MigrationReadinessPreflight` + a read-only CLI) that deterministically validates a `phase-40k-readiness.v1` manifest into `pass` / `blocked` / `fail_closed` with bounded non-secret evidence, and a separate fail-closed reviewed-execution gate (`ReviewedMigrationExecutionGate`) that defaults to refusal and cannot execute without exact, verified authorization bound to the exact manifest. The preflight touches no store, holder, ledger, snapshot, or dataset and performs no mutation; a passing preflight never authorizes execution. Reuses Phase 40I contracts (canonical identity, trusted-clock boundary, authorization semantics). Every real operational field stays `not_supplied` / `unverified` / `blocked`; no real dataset access, authorization creation, or production migration. Sequence: **40K.5 → 40K.6 → 40K.7 → 40L**; Phase 40L still requires a new explicit devdevbuilds go. |
| Phase 40K.6 — Real Dataset Identity + Backup/Restoration Readiness Verification | Completed and merged / operational readiness blocked | Planning/verification-readiness package ([Phase 40K.6 plan](operations/phase-40k-6-real-dataset-backup-restoration-readiness.md)) defining the real-dataset and backup/restoration evidence contract. No real dataset access, backup mutation, restoration, or production write occurred. Its `CONDITIONALLY READY` documentation verdict did not close B-01–B-13 or satisfy the 12 Phase 40L criteria. |
...
Phase 40E added the migration track's **intake boundary** (merged): the
`memory-migration.v1` contracts and a deterministic, read-only assessment that
judges a bundle's **declared metadata** and decides whether a future parser may
attempt it. Judging a declaration is not importing anything — Phase 40E opens no
artifact, reads no byte, and touches no filesystem, archive, Git remote, or
network.

Phase 40F adds the migration track's **export parser and candidate projector**
(merged): holding a matching Phase 40E
`ready_for_parsing` assessment, it reads the declared bytes, proves they match the
declared digest and size, safely parses the supported export and curated formats,
and projects user-visible conversational and document content into **inactive,
unverified, provenance-linked candidates**. Parsing is interpretation, not
verification, activation, human confirmation, or persistence: byte-integrity means
...
Phase 40H, whose **documentation-only** plan has **already selected** its durable
medium (the existing Active Memory store stays authoritative; one reviewed-import
coordinator inserts approved candidates through that store's own seam via a mandatory
authoritative live-store holder, using a reviewer-approved import specification that
supplies the record kind/claim/project the candidate does not carry, backed by a
separate durable versioned-JSON workflow ledger — which also owns an **authoritative
durable authorization-revocation registry** and evaluates authorization expiry against a
**trusted server-side UTC clock** — and a mandatory durable Active Memory snapshot store
bound to it by a shared persisted commit generation, serialized by a single coordinator
lock under a recoverable — never falsely atomic — cross-store protocol with an explicit
N/N+1 uncertain-commit recovery exception) with **no runtime, no public API, local and
unmerged, implementation locked**, and pending a **final independent seven-commit audit**
it has not passed; **no instruction remains to choose a durable medium**.
...
| Phase 40F — Export Parser + Candidate Projection | Implemented / merged | Backend-only export **parser** (`memory_migration_parser.py`, the only Phase 40F component permitted byte access) and **pure projector** (`memory_migration_projection.py`), with the candidate/result contracts in `models/memory_migration_projection.py`. Reuses the exact Phase 40E `assessment.permits_parsing(bundle_fingerprint=…)` gate — no byte is read before it passes and a stale/mismatched assessment authorizes nothing. Verifies actual byte integrity (recomputed accepted digest and size) against the declaration before trusting bytes, without mutating the Phase 40E declared-digest flag; parses `chatgpt_export_archive` (defensive in-memory ZIP, never extracted to disk), `chatgpt_conversations_json`, `curated_json_bundle` (versioned, `extra="forbid"`), `curated_markdown_bundle`, and `plain_text_document` deterministically; and projects only user-visible `user`/`assistant` and document content into **inactive, unverified, human-review-required, non-persistable, provenance-linked candidates** structurally bound by `CANDIDATE_MEMORY_POLICY`. Deterministic content-derived identity and canonical ordering; bounded content with deterministic chunking; honest content/count overflow; a closed, code-derived-severity diagnostic taxonomy carrying no raw exported content. No persistence, Active Memory insertion, verified/human-confirmed candidate, activation, endpoint, router, frontend, Source Registry/graph/repository/Git mutation, network, new dependency, or AI/LLM integration. See the [Phase 40F plan](planning/phase-40f-export-parser-candidate-projection.md). |
| Phase 40G — Migration Candidate Assessment (Dry-Run) | Implemented / merged | Backend-only deterministic, pure, read-only `MemoryMigrationCandidateAssessor` over a *complete* Phase 40F candidate set plus its projection diagnostics, producing a passive `MigrationCandidateAssessmentReport`: exact duplicates grouped and counted (never removed), conflicting source identities flagged as blocking, ambiguous source ordering and empty/degenerate candidates noted as advisory, unresolved Phase 40F errors carried as blocking and non-error overflow/truncation carried as advisory with original severity preserved, bounded aggregate coverage by the closed role/source-type vocabularies, a content-addressed tamper-evident report identity, and a derived `ready_for_review` / `review_with_warnings` / `blocked` verdict that fails closed. Phase 40F contracts reused unchanged. Nothing is persisted, imported, verified, approved, activated, ranked, deleted, or mutated; no endpoint, router, frontend, persistence, database, Active Memory, Grounded Synthesis runtime, graph, dependency, or AI/LLM integration; no clock, randomness, or I/O (AST-enforced). See the [Phase 40G plan](planning/phase-40g-migration-candidate-assessment-dry-run.md). |
| Phase 40H — Reviewed Persistence + Verified Import | Documentation-only planning (local, unmerged) / no runtime / no public API / pending a final independent seven-commit audit | First phase that will persist migrated candidates, and only for candidates a human has explicitly reviewed and approved. The [Phase 40H plan](planning/phase-40h-reviewed-persistence-verified-import-planning.md) is **documentation-only and has already selected its durable medium**: an orchestration over **three stores**: the **existing Phase 37C Active Memory store remains authoritative** for memory records; one reviewed-import **coordinator** is an orchestration boundary — the sole path that inserts an approved candidate through the Active Memory store's own seam; a separate durable ledger (`MigrationImportStore`) owns only migration-workflow records (review decisions, import attempts, receipts, **authorization revocation entries**, idempotency/recovery metadata) and **references the resulting `record_id` rather than copying the record**; and a **mandatory durable `ActiveMemorySnapshotStore`** owns the authoritative durable serialization of memory records, bound to the ledger by a shared persisted **`commit_generation`**. The plan also defines review-decision provenance (a bare approval boolean never authorizes mutation) with independent `supersedes_decision_id` lineage, a **mandatory reviewer-approved `ReviewedImportSpecification`** supplying the record `kind`, structured `claim`, and validated `project_id` that the Phase 40F `MemoryMigrationCandidate` does **not** carry (validated and executed with no semantic inference, kind selection, claim synthesis, or project guessing), a typed **project/scope authorization context** whose **expiration is evaluated by a trusted server-side UTC clock** (never a caller-supplied timestamp) and whose **revocation is an authoritative durable ledger registry** (never a caller-presented boolean), with a deterministic issuance discriminator and a single canonical `project_authorization_mismatch` diagnostic, a **mandatory `AuthoritativeActiveMemoryStoreHolder`** that owns the live Active Memory reference and performs atomic, reader-safe store replacement with a same-process synchronization seam, the exact candidate-to-`MemoryRecord` provenance mapping (a typed `migration_provenance` block; no Phase 37B model change), the persistence lifecycle (imported records are `INACTIVE`; the prohibited `INACTIVE → SUPERSEDED` transition is never invoked and `transition_lifecycle` is not called — supersession is expressed only through `supersession_refs`), the ordered verified-import preconditions (reload candidate, recompute/compare byte digest, confirm assessment identity/version, validate the reviewed specification, require complete non-contradictory non-superseded approval, fail closed), the full deterministic receipt contract referencing the exact `record_id` with **receipt identity separated from a full-content `receipt_integrity_digest` that covers both timestamps**, a **recoverable cross-store protocol that explicitly does not claim filesystem-level atomicity** (shared commit generation + durable intent + explicit recovery) including an **explicit N/N+1 uncertain-commit recovery exception evaluated before ordinary generation equality**, a **single coordinator lock using one concrete Windows-compatible dependency-free lock-file protocol** (atomic `O_EXCL` create) plus persisted-revision/CAS, canonical identity derivation for every derived id, a stable idempotency key (binding the specification digest) with distinct monotonic attempt ids scoped to retries, post-insert rollback + service quarantine, uncertain-commit recovery validating exact record equality, a normalized typed-diagnostic taxonomy (including `missing_reviewed_specification`, `invalid_reviewed_specification`, `missing_claim`, `malformed_claim`, `unsupported_claim_kind`, `kind_claim_incompatible`, `kind_claim_policy_version_mismatch`, `specification_binding_mismatch`, `project_authorization_mismatch`, `unauthorized_project_scope`, `authorization_context_revoked`, `authorization_context_expired`, `incomplete_authorization_lineage`, `specification_integrity_failure`, `live_store_replacement_failure`, `malformed_lock_metadata`, `lock_release_failure`, `stale_approval`, `corrupt_ledger`, `corrupt_active_memory_snapshot`, `snapshot_missing`, `generation_mismatch`, `uncertain_commit_result`, `missing_linked_attempt`, `missing_linked_memory_record`, `record_identity_collision`, `receipt_integrity_failure`, `lock_unavailable`, `stale_lock_ambiguous`, `revision_conflict`, `import_service_quarantined`), an expanded test matrix (including same-process multithreading and trusted-clock/durable-revocation coverage), and a mandatory, bounded implementation map naming the existing Active Memory modules it touches. It **reuses the established local versioned-JSON persistence architecture** (the Phase 39B atomic-write pattern) and explicitly **rejects a PostgreSQL/server-database rewrite**; **no new public API is proposed and no instruction remains to choose a durable medium**. Documentation-only: **no candidate is persisted or imported today**, nothing is implemented, the plan is **local and unmerged** (now **seven commits ahead of `origin/main`** after this hardening commit), and **implementation remains locked** until the planning branch passes a **final independent seven-commit Codex re-audit** — which it has **not** passed — and is merged. **Phase 36K remains paused and untouched.** Verified import is human-gated and byte-consistent; imported material is never treated as verified truth automatically. Successor: Phase 40I. |
| Phase 40I — Reviewed Persistence + Verified Import Implementation MVP | Implemented / merged | Implements the Phase 40H human-gated import contract with integrity-sealed workflow-ledger and Active Memory snapshot persistence, trusted-clock authorization, durable revocation, receipts, exact replay, recovery, and publish-last holder replacement. No public migration API or frontend. |
| Phase 40J — Migration Rehearsal + End-to-End Validation | Implemented / merged | Exercises Phase 40F–40I against deterministic synthetic disposable data, including missing authorization, integrity/CAS failures, exact replay, and N/N+1 recovery. It did not access or migrate authoritative data. See the [Phase 40J evidence](qa/phase-40j-migration-rehearsal-end-to-end-validation.md). |
| Phase 40K — Authoritative Migration Runbook + Execution Readiness | Implemented locally / operational readiness blocked / pending independent audit | Canonical human-operated [runbook](operations/phase-40k-authoritative-migration-runbook.md), [checklist](operations/phase-40k-operator-checklist.md), and blocked-by-default [readiness template](operations/phase-40k-readiness.template.json). Documentation/readiness only: no production write, attempt, receipt, authorization change, or real migration. |
| Phase 40K.5 — Production Migration Interface + Operational Readiness Remediation | Merged (PR #203) / operational readiness blocked / pending independent audit | Read-only [readiness preflight interface](operations/phase-40k-5-migration-readiness-interface.md) + fail-closed reviewed-execution gate that reconcile the `phase-40k-readiness.v1` manifest into a deterministic `pass`/`blocked`/`fail_closed` result and a refuse-by-default execution decision. No store/holder/ledger/dataset access, no mutation, and no execution possible without exact verified authorization; a passing preflight does not authorize Phase 40L. |
| Phase 40K.6 — Real Dataset Identity + Backup/Restoration Readiness Verification | Completed and merged / operational readiness blocked | Documentation-only [planning contract](operations/phase-40k-6-real-dataset-backup-restoration-readiness.md). No real evidence was supplied or operational action performed; B-01–B-13 remain open. |
| Phase 40K.7 — Final Operational Go/No-Go Review | Operational review conducted (branch-only) / result NO-GO / operational readiness blocked | The [authoritative review contract](operations/phase-40k-7-final-operational-go-no-go-review.md) defines the private packet, evidence mapping, roles, deterministic dispositions, blocker reconciliation, and all 12 Phase 40L criteria. The read-only evidence-based review executed under it — [Phase 40K.7 operational review result](operations/phase-40k-7-operational-go-no-go-review-result.md) — records a **NO-GO**: all 12 criteria remain OPEN, B-01–B-13 remain OPEN, and the committed readiness manifest evaluates to `blocked`. The review closes no blocker and authorizes nothing. Phase 40L remains locked. |
| Phase 40K.8 — Operational Blocker Remediation Plan | Documentation-only planning (branch) / operational readiness blocked | Documentation-only [remediation plan](operations/phase-40k-8-operational-blocker-remediation-plan.md) that maps every OPEN 40K.7 criterion and blocker B-01–B-13 to its authoritative requirement, closure rule, admissible out-of-Git evidence, owner, reviewer, dependencies, safe preparation steps, private evidence-packet destination, freshness rule, validation method, signatures, remediation waves, and re-review entry gate. Remediation planning only: no remediation performed, no operational evidence collected, no production access, no blocker closed, and no criterion satisfied. B-01–B-13 and all 12 criteria remain OPEN; Phase 40L stays locked. |
| Phase 40L — Authorized Production Migration Execution + Acceptance Evidence | Conditional / blocked | May begin only after a separate explicit human go decision verifies the real dataset identity, destination revision, backup/restoration proof, exact live authorization, stop conditions, evidence destinations, and approved production preflight/execution interface. A Phase 40K repository review does not authorize it. |
...
- [Phase 40F Export Parser + Candidate Projection](planning/phase-40f-export-parser-candidate-projection.md)
- [Phase 40G Migration Candidate Assessment (Dry-Run)](planning/phase-40g-migration-candidate-assessment-dry-run.md)
- [Phase 40H Reviewed Persistence + Verified Import (Planning)](planning/phase-40h-reviewed-persistence-verified-import-planning.md)
