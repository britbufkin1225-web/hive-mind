...
Phase 40H, whose documentation-only plan proposes a two-store orchestration (the
existing Active Memory store stays authoritative; a new reviewed-import service
inserts approved candidates through that store's own seam, backed by a separate
durable versioned-JSON workflow ledger) with no runtime implemented.
...
| Phase 40H — Reviewed Persistence + Verified Import | Planning in progress (documentation-only) / no runtime implemented | First phase that will persist migrated candidates, and only for candidates a human has explicitly reviewed and approved. The [Phase 40H plan](planning/phase-40h-reviewed-persistence-verified-import-planning.md) *proposes* a **two-store model**: the **existing Phase 37C Active Memory store remains authoritative** for memory records, and a new reviewed-import service is an **orchestration boundary** — the sole path that inserts an approved candidate through the Active Memory store's own seam, while a separate durable ledger (`MigrationImportStore`) owns only migration-workflow records (review decisions, import attempts, receipts, idempotency/recovery metadata) and **references the resulting `record_id` rather than copying the record**. The plan also defines review-decision provenance (a bare approval boolean never authorizes mutation), the persistence lifecycle, the verified-import preconditions (reload candidate, recompute/compare byte digest, confirm assessment identity/version, require complete non-contradictory approval, fail closed), the deterministic import receipt referencing the exact `record_id`, a **recoverable cross-store transaction protocol that explicitly does not claim single-swap atomicity**, an exclusive-writer + persisted-revision/CAS concurrency mechanism, a stable idempotency key with distinct monotonic attempt ids, uncertain-commit recovery, typed diagnostics (including `stale_approval`, `corrupt_ledger`, `uncertain_commit_result`, `missing_linked_attempt`, `missing_linked_memory_record`, `revision_conflict`), a test matrix, and an integration map naming the existing Active Memory modules it touches. It **proposes reusing the established local versioned-JSON persistence architecture** (the Phase 39B atomic-write pattern) and explicitly **rejects a PostgreSQL/server-database rewrite**; no new public API is proposed. Planning-only: **no candidate is persisted or imported today**, nothing is implemented, and implementation requires separate approval. Verified import is human-gated and byte-consistent; imported material is never treated as verified truth automatically. Successor: Phase 40I. |
