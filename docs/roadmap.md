...
| Phase 40H — Reviewed Persistence + Verified Import | Planning in progress (documentation-only) / no runtime implemented | First phase that will persist migrated candidates, and only for candidates a human has explicitly reviewed and approved. The [Phase 40H plan](planning/phase-40h-reviewed-persistence-verified-import-planning.md) defines the authoritative state owner and exclusive mutation boundary (a single reviewed-import service), the durable record types and their relationships, review-decision provenance (a bare approval boolean never authorizes mutation), the persistence lifecycle, the verified-import preconditions (reload candidate, recompute/compare byte digest, confirm assessment identity/version, require complete non-contradictory approval, fail closed), the deterministic import receipt, idempotency/replay and atomicity/recovery rules, typed diagnostics, a test matrix, and a tightly bounded implementation file map. It **proposes reusing the established local versioned-JSON persistence architecture** (the Phase 39B atomic-write pattern) and explicitly **rejects a PostgreSQL/server-database rewrite**; no new public API is proposed. Planning-only: **no candidate is persisted or imported today**, nothing is implemented, and implementation requires separate approval. Verified import is human-gated and byte-consistent; imported material is never treated as verified truth automatically. Successor: Phase 40I. |
